...
![Installations list](./img/installation-list.png)
...
![Installations details](./img/installation-details.png)
...
_Assets tab will be expanded with additional data regarding specific assets._

**Tasks**
Task tab with a list of tasks to be performed in this location by installers. Task can be edited or deleted from installation only if its status is pending. Each task contains name, due date and it's state.

_Tasks are not fully implemented yet.
Planned extensions of their functionality include using them to determine assets to be installed in a given location and monitor the progress of this work._
